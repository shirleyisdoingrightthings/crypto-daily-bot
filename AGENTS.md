...
### 消息②结构
- 开头是「📋 加密市场播报 · 日期」，紧跟「⚡ 30秒速览」（固定 5 条、行首带星级、每条 ≤55 字且必须含具体数字或专有名词、与正文前 5 条一一对应），再进正文条目
- 速览规则写在 `prompt_news.md` 的输出格式里，改版式改那里

### 行情与新闻的不同频率（2026-08 起）

| 内容 | 频率 | 理由 |
|---|---|---|
| 消息①行情晨报 | **每天** | 价格/恐惧指数/板块轮动是易腐数据，隔天就失去参考价值 |
| 消息②新闻播报 | **每 3 天** | 新闻时间窗本就是 3 天，每天播意味着同一条新闻有 3 次被选中的机会——这是重复的根源。改成 3 天一播后窗口与频率刚好对齐，零重叠零遗漏 |

- 由 `logs/.last_news` 记录上次实际播出新闻的日期，`_news_due()` 按**实际经过天数**判断（非固定周期），某次没跑成时下次会自动补上
- 未到期时 `gather()` **整段跳过 RSS 抓取**，省时间也省写稿 token
- `fetch` 的 stdout 输出 `=== NEWS_INCLUDED ===` 或 `=== NEWS_SKIPPED ===`，routine 据此决定写一稿还是两稿
- `send` 从 `fetch_meta.json` 的 `news_included` 判断发一条还是两条，并对新闻稿做**当日新鲜度校验**——非新闻日磁盘上留着几天前的旧稿，无脑发送会造成旧闻重播
- `auto_repair.sh` 的 `DRAFTS` 也随该标记动态变化，否则非新闻日会因旧新闻稿过期而误判"当日稿件缺失"
